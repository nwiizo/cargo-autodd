...
fn test_monorepo_dependency_analysis() -> Result<()> {
    ...
    // 結果の検証
    // tokioが検出されるべき
    assert!(crate_refs.contains_key("tokio"), "tokio dependency should be detected");
    
    // internal-crateまたはinternal_crateが検出されるべき
    // Rustの命名規則により、ハイフンはアンダースコアに変換されることがある
    let has_internal_crate = crate_refs.contains_key("internal-crate") || crate_refs.contains_key("internal_crate");
    assert!(has_internal_crate, "internal crate dependency should be detected");
...
fn test_monorepo_update_with_internal_crates() -> Result<()> {
    ...
    // 結果の検証
    assert!(result.is_ok(), "analyze_and_update should succeed with internal crates");
    
    // 更新後のCargo.tomlを読み込む
    let cargo_toml_content = fs::read_to_string(root_path.join("public-crate/Cargo.toml"))?;
    
    // tokioが追加されているか確認
    assert!(cargo_toml_content.contains("tokio"), "tokio should be added to Cargo.toml");
    
    // internal-crateがpath依存として維持されているか確認
    assert!(cargo_toml_content.contains("internal-crate = { path ="), 
...
fn test_monorepo_with_publish_false_crates() -> Result<()> {
    ...
    // 結果の検証
    assert!(result.is_ok(), "analyze_and_update should succeed with publish=false crates");
    
    // 更新後のCargo.tomlを読み込む
    let cargo_toml_content = fs::read_to_string(root_path.join("internal-crate/Cargo.toml"))?;
    
    // publish = falseが維持されているか確認
    assert!(cargo_toml_content.contains("publish = false"), 
            "publish = false should be maintained");
    
    // serdeが追加/維持されているか確認
    assert!(cargo_toml_content.contains("serde"), "serde should be in Cargo.toml");
